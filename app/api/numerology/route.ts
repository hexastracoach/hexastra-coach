...
if (NORMALIZED_API_BASE.warning === 'railway_http_upgraded_to_https') {
  ...
  logger.warn('[numerology] HEXASTRA_API_URL normalized', {
    rawValue: NORMALIZED_API_BASE.input,
    normalizedUrl: NORMALIZED_API_BASE.url,
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.error('[numerology] backend error', {
        url,
    ...
    logger.error('Numerology proxy error', { error: e })
