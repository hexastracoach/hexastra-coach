...
export async function POST(req: NextRequest) {
  ...
  const log = (level: 'info' | 'warn' | 'error', msg: string, meta?: Record<string, unknown>) =>
    logger[level](`[stripe:webhook][${requestId}] ${msg}`, meta)
  ...
  try {
    ...
    log('error', 'Signature Stripe invalide', { err })
    return NextResponse.json({ error: 'Signature invalide' }, { status: 400 })
  }

  if (!STRIPE_EVENTS_HANDLED.has(event.type)) {
    log('info', 'Stripe event ignoré', { type: event.type })
  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        log('info', 'Checkout session completed', { sessionId: session.id, userId, priceKey, plan })
        ...
        if (supabase && userId) {
          ...
          if (profileError) log('error', 'Profile update after checkout failed', { profileError })
          ...
          if (orderError) log('warn', 'Order update ignored/failed', { orderError })
        }
        break
      }
      ...
      case 'customer.subscription.updated': {
        ...
        log('info', 'Subscription upsert', { event: event.type, subscriptionId, customerId, userId, priceKey, plan, subscriptionStatus })
        ...
        if (supabase) {
          ...
          if (error) log('error', 'Profile update failed on subscription event', { error })
        }
        break
      }
      ...
      case 'customer.subscription.deleted': {
        ...
        if (supabase) {
          ...
          if (subscriptionId) {
            ...
            if (error) log('error', 'Downgrade by subscription_id failed', { error })
          }

          if (customerId) {
            const { error } = await supabase.from('profiles').update(update).eq('stripe_customer_id', customerId)
            if (error) log('error', 'Downgrade by customer_id failed', { error })
          }
        }
        break
      }
      ...
      case 'invoice.paid': {
        ...
        log('info', 'Invoice paid', { invoiceId: invoice.id, customerId })
        ...
        if (supabase && customerId) {
          ...
          if (error) log('error', 'Update status active failed', { error })
        }
        break
      }
      ...
      case 'invoice.payment_failed': {
        const invoice = event.data.object as Stripe.Invoice
        const customerId = typeof invoice.customer === 'string' ? invoice.customer : invoice.customer?.id || null
        log('warn', 'Invoice payment failed', { invoiceId: invoice.id, customerId })
        ...
        if (supabase && customerId) {
          const { error } = await supabase
            ...
            .eq('stripe_customer_id', customerId)
          if (error) log('error', 'Update status past_due failed', { error })
        }
        break
      }
    ...
    log('error', 'Erreur webhook Stripe', { error: err })
