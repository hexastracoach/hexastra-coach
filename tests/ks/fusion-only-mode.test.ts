...
describe('fusion-only mode', () => {
  ...
  it('injects plan-specific yearly priority instructions into the annual prompt', () => {
    ...
    expect(freePrompt).toContain('titre court seulement')
    expect(freePrompt).toContain('Evite Pourquoi detaille')
    ...
    expect(essentialPrompt).toContain('Utilise des verbes simples')
    expect(essentialPrompt).toContain('Evite les formules abstraites')
    expect(essentialPrompt).toContain('Cle simple: optionnelle')
    expect(essentialPrompt).toContain('Priorite radicale: optionnelle')
    expect(essentialPrompt).toContain('ACTION IMMEDIATE: donne 1 a 2 actions maximum')
    expect(essentialPrompt).toContain('Jamais plus de 2 actions')
